...
import { TextElementBuilder } from "@here/harp-mapview/lib/text/TextElementBuilder";
import {
    EdgeMaterial,
        ...
        if (decodedTile.textPathGeometries !== undefined) {
            ...
            for (const textPath of textPathGeometries) {
                ...
                const textElement = textElementBuilder
                    ...
                    .build(textPath.text, path, tile, textPath.objInfos, textPath.pathLengthSqr);
        ...
        if (decodedTile.textGeometries !== undefined) {
            for (const text of decodedTile.textGeometries) {
                ...
                for (let i = 0; i < numPositions; ++i) {
                    ...
                    const textElement = textElementBuilder.build(label!, point, tile, attributes);
                    tile.addTextElement(textElement);
                }
            }
        }
